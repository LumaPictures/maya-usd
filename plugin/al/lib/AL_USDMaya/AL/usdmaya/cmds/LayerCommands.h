...
public:
  ...
  /// \brief  sets up some core/common command params for all the layer commands. Just specifies that selection can be
  ///         used as an argument.
...
private:
  ...
  MStatus undoIt() override;
  MStatus redoIt() override;
  PcpNodeRef determineEditTargetMapping(UsdStageRefPtr stage, const MArgDatabase& args, SdfLayerHandle editTargetLayer) const;
};
