...
//----------------------------------------------------------------------------------------------------------------------
bool ChangeVariant::isUndoable() const
{
  return false;
}
...
MStatus ChangeVariant::doIt(const MArgList& args)
{
  try
  {
    ...
    if(!database.getFlagArgument("-pp", 0, pp) ||
       !database.getFlagArgument("-vs", 0, vset) ||
...
//----------------------------------------------------------------------------------------------------------------------
bool ActivatePrim::isUndoable() const
{
  return false;
}
...
MStatus ActivatePrim::doIt(const MArgList& args)
{
  try
  {
    ...
    if(!database.getFlagArgument("-pp", 0, pp) ||
       !database.getFlagArgument("-a", 0, active))
    {
      MGlobal::displayError("Not enough information to activate prim");
      return MS::kFailure;
    }
