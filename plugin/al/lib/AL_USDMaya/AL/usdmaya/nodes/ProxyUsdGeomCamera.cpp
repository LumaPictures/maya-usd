...
bool ProxyUsdGeomCamera::getInternalValue(const MPlug &plug, MDataHandle &dataHandle)
{
  ...
  if (plug == m_nearClipPlane)
  {
    ...
    if (attr.GetVariability() != SdfVariability::SdfVariabilityVarying) usdTime = UsdTimeCode::Default();
    GfVec2f value;
...
bool ProxyUsdGeomCamera::setInternalValue(const MPlug &plug, const MDataHandle &dataHandle)
{
  ...
  else if (plug == m_stereoRole)
  {
    ...
    switch(mode)
    {
    case (StereoRole::Mono):
      handledAttribute = attr.Set(UsdGeomTokens->mono, usdTime);
      break;
    case (StereoRole::Left):
      handledAttribute = attr.Set(UsdGeomTokens->left, usdTime);
      break;
    case (StereoRole::Right):
      handledAttribute = attr.Set(UsdGeomTokens->right, usdTime);
      break;
    }
  }
  else if (plug == m_verticalAperture)
...
MStatus ProxyUsdGeomCamera::initialise()
{
  ...
  const int16_t projectionValues[] = { TO_MAYA_ENUM(Projection::Perspective), TO_MAYA_ENUM(Projection::Orthographic) };
  m_projection = addEnumAttr("projection", "pron", defaultFlags, projectionKeys, projectionValues);
  ...
  const int16_t stereoRoleValues[] = { TO_MAYA_ENUM(StereoRole::Mono), TO_MAYA_ENUM(StereoRole::Left), TO_MAYA_ENUM(StereoRole::Right) };
  m_stereoRole = addEnumAttr("stereoRole", "stee", defaultFlags, stereoRoleKeys, stereoRoleValues);
  ...
  m_nearClipPlane = addDistanceAttr("nearClipPlane", "ncp", MDistance(0.1, MDistance::kCentimeters), defaultFlags | kHidden);
  MFnNumericAttribute(m_nearClipPlane).setMin(0.001);
  attributeAffects(m_path, m_nearClipPlane);
  
  m_farClipPlane = addDistanceAttr("farClipPlane", "fcp", MDistance(10000.0, MDistance::kCentimeters), defaultFlags | kHidden);
  MFnNumericAttribute(m_farClipPlane).setMin(0.001);
  attributeAffects(m_path, m_farClipPlane);

  m_clippingRange = addCompoundAttr("clippingRange", "cr", defaultFlags, { m_nearClipPlane, m_farClipPlane });
  MFnAttribute(m_clippingRange).setNiceNameOverride("Clipping Range");
  
  addFrame("Film Back");

  m_horizontalFilmAperture = addDoubleAttr("horizontalFilmAperture", "hfa", 0.0, defaultFlags | kHidden);
  ...
  m_verticalFilmAperture = addDoubleAttr("verticalFilmAperture", "vfa", 0.0, defaultFlags | kHidden);
  ...
  m_cameraApertureInch = addCompoundAttr("cameraAperture", "cap", defaultFlags, { m_horizontalFilmAperture, m_verticalFilmAperture });
  ...
  m_cameraApertureMm = addCompoundAttr("cameraApertureMm", "capm", defaultFlags, { m_horizontalAperture, m_verticalAperture });
  MFnAttribute(m_cameraApertureMm).setNiceNameOverride("Camera Aperture (mm)");
  
  m_horizontalFilmOffset = addDoubleAttr("horizontalFilmOffset", "hfo", 0.0, defaultFlags | kHidden);
  ...
  m_filmOffset = addCompoundAttr("filmOffset", "fio", defaultFlags, { m_horizontalFilmOffset, m_verticalFilmOffset });
  MFnAttribute(m_filmOffset).setNiceNameOverride("Film Offset (inch)");
  
  m_horizontalApertureOffset = addFloatAttr("horizontalApertureOffset", "hao", 0.0f, readOnlyFlags | kHidden);
  ...
  m_verticalApertureOffset = addFloatAttr("verticalApertureOffset", "vao", 0.0f, readOnlyFlags | kHidden);
  ...
  m_focusDistance = addDistanceAttr("focusDistance", "fd", MDistance(5.0, MDistance::kCentimeters), defaultFlags);
