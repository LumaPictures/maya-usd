...
public:
  ...
  /// \brief  Either find the already-existing non-referenced RendererManager node in the scene, or make one
  ...
  /// \brief  Find the already-existing non-referenced RendererManager node in the scene, or return a nullptr
  /// \return the found RendererManager, or a nullptr
  static RendererManager* findManager();

  /// \brief  Either find the already-existing non-referenced RendererManager in the scene, or make one
  /// \param dgmod An optional dgmodifier to create the node, if necessary. Note that if one is passed in,
  ///              createNode might be called on it, but doIt never will be, so the layer manager node
  ///              may not be added to the scene graph yet
  /// \param wasCreated If given, whether a new layer manager had to be created is stored here.
