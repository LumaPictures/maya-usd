...
/// \brief  Callback type used to provide a list of *things* to an optionMenu control in an option box GUI
...
/// \ingroup mayagui
//----------------------------------------------------------------------------------------------------------------------
class CommandGuiListGen
  : public MPxCommand
{
public:
  AL_MAYA_DECLARE_COMMAND();

  /// \brief  Internal method. Registers a custom list function with this command so that MEL can request a custom C++
  ///         command to generate a list of items, which are then displayed within a MEL GUI.
  /// \param  generateListFunc a C++ function that can take an optional user specified 'context', and return a list of
...
//----------------------------------------------------------------------------------------------------------------------
/// \brief  This class isn't really a wrapper around command options as such, it's mainly just a helper
///         to auto generate some GUI code to create a menu item + option box dialog.
/// \note
/// The following example code demonstrates how to use the CommandGuiHelper class to autogenerate a menu item,
/// which will be available in the menu path  "USD" -> "polygons" -> "Create Cube"; This will call the mel command "polyCube".
/// The total command called will be something akin to:
///
...
///
/// However all of the numeric values will actually be stored as optionVar's. (see the optionVar mel command, or MGlobal class)
/// If the command is "polyCube", and the flag is "constructionHistory", then the optionVar used to store the preference
/// will be "polyCube_constructionHistory".
///
/// Whilst I'm using "polyCube" as an example of how to use this class, you'd probably want to use this for your own
/// MPxCommand derived classes.
///
/// \code
/// {
...
/// \endcode
///
/// If the above code is called somewhere within your initialisePlugin method, and you end up calling
/// AL::maya::MenuBuilder::generatePluginUI() at the end of your initialise method, then that command (+optionBox) will
/// be available on the main maya menu.
/// \ingroup mayagui
//----------------------------------------------------------------------------------------------------------------------
...
public:
  ...
  /// \brief  Used to describe the type of file dialog that should be used for a file path attribute
  enum FileMode
  {
  ...
  /// \brief  determines if a text string argument is optional (e.g. a name of the object if specified, but falls back
  ...
  /// \param  doitLabel    The label that will appear on the 'Create/Yes/DoIt' button on the left hand side of the dialog
  /// \param  menuItemPath determines the path to the menu item from the main menu.
  /// \param  hasOptionBox If true, the dialog will only be executed when the option box button is clicked.
  ///                      If false, there will be no option box, and the GUI will always be displayed.
  AL_MAYA_UTILS_PUBLIC
  ...
  /// \brief  adds a dynamic drop down list of items that will be displayed within an optionMenu control.
  ///         The list of text strings will be generated within C++ by the custom generateList function, which will then
  ///         be chosen to represent the list of options available for this command. When the command is executed, the
  ///         selected item will be passed to the command as a text string.
  /// \note   Only string command options are supported for this control type, and the command GUI must be created with
  ...
  /// \param  enumNames   an array of text string names (last string must be NULL) for the enum entries. e.g.
  /// \code
  ...
  ///   };
  /// \endcode
  /// \param  enumValues   an array of integer values that match up to the enumNames. This array can be NULL,
  ///         in which case it is assumed that the enum values are 0, 1, 2, 3, etc.
  /// \param  persist   if true, the GUI option will be persisted as an optionVar
  /// \param  passAsString   if true, the command will be passed the text string value of the enum. If false,
  ...
  /// \brief  Similar to the enum option, but this time with radio buttons. THE MAXIMUM NUMBER OF OPTIONS IS 4.
  ...
  /// \param  defaultVal   the default value for the flag if not set
  /// \param  persist   if true, the GUI option will be persisted as an optionVar
  /// \param  policy   is the string optional (kStringOptional), or required? (kStringMustHaveValue)
  AL_MAYA_UTILS_PUBLIC
  ...
  /// \param  policy   is the file path optional (kStringOptional), or required? (kStringMustHaveValue)
  AL_MAYA_UTILS_PUBLIC
