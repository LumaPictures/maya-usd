...
std::map<std::string, PluginTranslatorOptionsContext*> PluginTranslatorOptionsContextManager::m_contexts;
...
void PluginTranslatorOptionsContext::registerPluginTranslatorOptions(PluginTranslatorOptions* options)
{
...
//----------------------------------------------------------------------------------------------------------------------
PluginTranslatorOptionsArray::iterator find(PluginTranslatorOptionsArray& array, const char* const groupName)
{
...
//----------------------------------------------------------------------------------------------------------------------
PluginTranslatorOptionsArray::const_iterator find(const PluginTranslatorOptionsArray& array, const char* const groupName)
{
  for(auto it = array.begin(); it != array.end(); ++it)
  {
    if((*it)->grouping() == groupName)
...
//----------------------------------------------------------------------------------------------------------------------
std::vector<PluginTranslatorOptions::Option>::iterator find(std::vector<PluginTranslatorOptions::Option>& array, const char* const groupName)
{
  for(auto it = array.begin(); it != array.end(); ++it)
  {
...
//----------------------------------------------------------------------------------------------------------------------
std::vector<PluginTranslatorOptions::Option>::const_iterator find(const std::vector<PluginTranslatorOptions::Option>& array, const char* const groupName)
{
  for(auto it = array.begin(); it != array.end(); ++it)
  {
    if(it->name == groupName)
...
//----------------------------------------------------------------------------------------------------------------------
AL_MAYA_UTILS_PUBLIC
void PluginTranslatorOptionsContext::unregisterPluginTranslatorOptions(const char* const pluginTranslatorGrouping)
{
...
//----------------------------------------------------------------------------------------------------------------------
AL_MAYA_UTILS_PUBLIC
bool PluginTranslatorOptions::addEnum(const char* optionName, const char* const enumValues[], int defaultValue)
{
  if(isOption(optionName))
    return false;
...
//----------------------------------------------------------------------------------------------------------------------
AL_MAYA_UTILS_PUBLIC
PluginTranslatorOptionsInstance::PluginTranslatorOptionsInstance(PluginTranslatorOptionsContext& context)
{
    ...
    for(uint32_t j = 0; j < m_options.size(); ++j)
    {
      ...
      if(opt->name == optName)
      {
        ...
        case OptionType::kBool:
          m_options[j].m_bool = optValue.asInt() != 0;
          break;

        case OptionType::kInt:
        case OptionType::kEnum:
          m_options[j].m_int = optValue.asInt();
          break;

        case OptionType::kFloat:
          m_options[j].m_float = optValue.asFloat();
          break;

        case OptionType::kString:
          m_options[j].m_string = optValue;
          break;
        }
        break;
      }
...
//----------------------------------------------------------------------------------------------------------------------
void PluginTranslatorOptions::generateBoolGlobals(const char* const prefix, const MString& niceName, const MString& optionName, MString& code, bool value)
{
  MString valueStr;
  valueStr =  int(value);
...
//----------------------------------------------------------------------------------------------------------------------
void PluginTranslatorOptions::generateIntGlobals(const char* const prefix, const MString& niceName, const MString& optionName, MString& code, int value)
{
  MString valueStr;
  valueStr = value;
  MString controlName = MString(prefix) + "_" + makeName(optionName);
...
//----------------------------------------------------------------------------------------------------------------------
void PluginTranslatorOptions::generateFloatGlobals(const char* const prefix, const MString& niceName, const MString& optionName, MString& code, float value)
{
  MString valueStr;
  valueStr = value;
  MString controlName = MString(prefix) + "_" + makeName(optionName);
...
//----------------------------------------------------------------------------------------------------------------------
void PluginTranslatorOptions::generateStringGlobals(const char* const prefix, const MString& niceName, const MString& optionName, MString& code, const char* value)
{
  MString controlName = MString(prefix) + "_" + makeName(optionName);
...
//----------------------------------------------------------------------------------------------------------------------
void PluginTranslatorOptions::generateEnumGlobals(const char* const prefix, const MString& niceName, const MString& optionName, const MString enumValues[], uint32_t enumCount, MString& code, int value)
{
  MString controlName = MString(prefix) + "_" + makeName(optionName);
...
MString PluginTranslatorOptions::generateGUI(const char* const prefix, MString& guiCode)
{
  for(uint32_t j = 0; j < m_options.size(); ++j)
  {
    ...
    switch(opt->type)
    {
    case OptionType::kBool: generateBoolGlobals(prefix, opt->name, opt->name, guiCode, opt->defBool); break;
    case OptionType::kInt: generateIntGlobals(prefix, opt->name, opt->name, guiCode, opt->defInt); break;
    case OptionType::kFloat: generateFloatGlobals(prefix, opt->name, opt->name, guiCode, opt->defFloat); break;
    case OptionType::kString: generateStringGlobals(prefix, opt->name, opt->name, guiCode, opt->defString.asChar()); break;
    case OptionType::kEnum: generateEnumGlobals(prefix, opt->name, opt->name, opt->enumStrings.data(), opt->enumStrings.size(), guiCode, opt->defInt); break;
    }
  }
