...
MStatus CommandGuiListGen::doIt(const MArgList& args)
{
  ...
  if(status)
  {
    ...
    {
      result += append_begin + strings[i] + append_mid + m_funcs[genListId].first + append_end;
    }
    setResult(result);
  }
  return status;
...
//----------------------------------------------------------------------------------------------------------------------
CommandGuiHelper::CommandGuiHelper(const char* commandName, const char* menuItemPath, bool checkBoxValue) : m_checkBoxCommand(true)
{
  m_hasFilePath = false;
...
//----------------------------------------------------------------------------------------------------------------------
CommandGuiHelper::CommandGuiHelper(const char* commandName, const char* windowTitle, const char* doitLabel, const char* menuItemPath, bool hasOptionBox) : m_checkBoxCommand(false)
{
  m_hasFilePath = false;
  ...
  if(hasOptionBox)
  {
    MenuBuilder::addEntry(menuItemPath, (std::string("execute_") + mycmd_optionGUI).c_str(), (std::string("build_") + mycmd_optionGUI).c_str());
  }
  else
  {
...
CommandGuiHelper::~CommandGuiHelper()
{
  ...
  // if we happen to have a file dialog knocking about in our GUI, ensure the handler is available.
  if(m_hasFilePath)
  {
...
void CommandGuiHelper::addFlagOption(const char* commandFlag, const char* label, const bool defaultVal, bool persist)
{
  ...
  // if the default value does not exist, force it to exist using the default value. Then set the control value
  if(persist)
  {
...
//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addBoolOption(const char* commandFlag, const char* label, const bool defaultVal, bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
...
void CommandGuiHelper::addListOption(const char* commandFlag, const char* label, GenerateListFn generateList, bool isMandatory)
{
  ...
  // register callback
  const int32_t id = CommandGuiListGen::registerListFunc(generateList, AL::maya::utils::convert(optionVar));
...
//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addIntOption(const char* commandFlag, const char* label, const int defaultVal, bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
...
//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addIntSliderOption(const char* commandFlag, const char* label, const int minVal, const int maxVal, const int defaultVal, bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
...
//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addInt2Option(const char* commandFlag, const char* label, const int32_t defaultVal[2], bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
  ...
  {
    m_init << "  if(!`optionVar -ex \"" << optionVar << "\"`)\n";
    m_init << "    optionVar -iv \"" << optionVar << "\" " << defaultVal[0]
                       << " -iva \"" << optionVar << "\" " << defaultVal[1] << ";\n";
  ...
  {
    // just set the checkbox to the default value
    m_load << "  intFieldGrp -e -v1 " << defaultVal[0]
                           << " -v2 " << defaultVal[1]
                           << " -v3 " << defaultVal[2] << " " << optionVar << ";\n";
  ...
  // reset checkbox back to the default value
  m_reset << "  intFieldGrp -e -v1 " << defaultVal[0]
                          << " -v2 " << defaultVal[1] << " " << optionVar << ";\n";
}

//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addInt3Option(const char* commandFlag, const char* label, const int32_t defaultVal[3], bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
  ...
  if(persist)
  {
    ...
    m_save << "  optionVar -iv \"" << optionVar << "\" `intFieldGrp -q -v1 " << optionVar
                    << "` -iva \"" << optionVar << "\" `intFieldGrp -q -v2 " << optionVar
                    << "` -iva \"" << optionVar << "\" `intFieldGrp -q -v3 " << optionVar << "`;\n";
...
//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addInt4Option(const char* commandFlag, const char* label, const int32_t defaultVal[4], bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
  ...
  if(persist)
  {
    ...
    m_save << "  optionVar -iv \"" << optionVar << "\" `intFieldGrp -q -v1 " << optionVar
                    << "` -iva \"" << optionVar << "\" `intFieldGrp -q -v2 " << optionVar
                    << "` -iva \"" << optionVar << "\" `intFieldGrp -q -v3 " << optionVar
                    << "` -iva \"" << optionVar << "\" `intFieldGrp -q -v4 " << optionVar << "`;\n";
    ...
    bool passAsString)
{
  ...
  if(persist)
  {
    ...
    // pull value from checkbox, and insert into the optionVar
    m_save << "  optionVar -iv \"" << optionVar << "\" (`optionMenu -q -sl " << optionVar << "` - 1);\n";
    ...
    bool passAsString)
{
  ...
  int32_t optionCount = 0;
  {
    ...
    {
      // if for some reason this is being called with more than 4 options, just use a combo box instead.
      if(optionCount > 4)
      {
        addEnumOption(commandFlag, label, defaultIndex, enumNames, enumValues, persist, passAsString);
        return;
      }
    }
    --optionCount;
  ...
  if(persist)
  {
    ...
    // pull value from checkbox, and insert into the optionVar
    m_save << "  optionVar -iv \"" << optionVar << "\" (`radioButtonGrp -q -sl " << optionVar << "` - 1);\n";
...
//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addDoubleOption(const char* commandFlag, const char* label, const double defaultVal, bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
...
//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addDoubleSliderOption(const char* commandFlag, const char* label, const double minVal, const double maxVal, const double defaultVal, bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
...
//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addVec2Option(const char* commandFlag, const char* label, const double defaultVal[2], bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
  ...
  {
    m_init << "  if(!`optionVar -ex \"" << optionVar << "\"`)\n";
    m_init << "    optionVar -fv \"" << optionVar << "\" " << defaultVal[0]
                       << " -fva \"" << optionVar << "\" " << defaultVal[1] << ";\n";
  ...
  {
    // just set the checkbox to the default value
    m_load << "  floatFieldGrp -e -v1 " << defaultVal[0]
                             << " -v2 " << defaultVal[1] << " " << optionVar << ";\n";
  ...
  // reset checkbox back to the default value
  m_reset << "  floatFieldGrp -e -v1 " << defaultVal[0]
                            << " -v2 " << defaultVal[1] << " " << optionVar << ";\n";
}

//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addVec3Option(const char* commandFlag, const char* label, const double defaultVal[3], bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
...
//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addVec4Option(const char* commandFlag, const char* label, const double defaultVal[4], bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
...
//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addColourOption(const char* commandFlag, const char* label, const double defaultVal[3], bool persist)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
  ...
  {
    // just set the checkbox to the default value
    m_load << "  colorSliderGrp -e -rgb " << defaultVal[0]
                                   << " " << defaultVal[1]
  ...
  // reset checkbox back to the default value
  m_reset << "  colorSliderGrp -e -rgb " << defaultVal[0]
                                  << " " << defaultVal[1]
                                  << " " << defaultVal[2] << " " << optionVar << ";\n";
}

//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addStringOption(const char* commandFlag, const char* label, MString defaultVal, bool persist, StringPolicy policy)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;
  const std::string getOptionVar = std::string("`optionVar -q \"") + optionVar + "\"`";
  ...
  if(persist)
  {
    ...
    if(policy == kStringMustHaveValue)
    {
      m_execute << "  if(!size(" << getOptionVar << ")) {\n";
...
//----------------------------------------------------------------------------------------------------------------------
void CommandGuiHelper::addFilePathOption(const char* commandFlag, const char* label, FileMode fileMode, const char* filter, StringPolicy policy)
{
  const std::string optionVar = m_commandName + "_" + commandFlag;

  // generate checkbox control for boolean argument
  m_controls << "  textFieldButtonGrp -h 20 -bl \"...\" -bc \"alFileDialogHandler(\\\"" << filter << "\\\", \\\"" << optionVar << "\\\", " << fileMode << ")\" " << optionVar << ";\n";
