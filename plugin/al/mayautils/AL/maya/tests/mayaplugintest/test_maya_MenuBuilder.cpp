...
// Make sure that when we add menu item paths under the same menu path, the entries get added correctly
...
{
  AL::maya::utils::MenuBuilder::clearRootMenus();
  EXPECT_TRUE(0 != AL::maya::utils::MenuBuilder::addEntry("FOO/BAR/MOO/HI1", "Checky", true, false));
  EXPECT_TRUE(0 != AL::maya::utils::MenuBuilder::addEntry("FOO/BAR/MOO/HI2", "McCheckFace", true, true));
  EXPECT_EQ(size_t(1), AL::maya::utils::MenuBuilder::rootMenus().size());
