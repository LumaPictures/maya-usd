...
{
  // Currently this tests all of the individual data types, all using the 'persist' parameter to ensure they get added
  // into the optionVars correctly, and can be retrieved and set by the gui. There aren't exactly loads of tests regarding
  // the non-persistent command args. I will need to go test those at some point.
  AL::maya::utils::CommandGuiHelper commandOpts("AL_usdmaya_CommandGuiHelperTest", "Unit Testy Stuff", "Test", "USD/Tests/CommandGuiHelperTest");
  ...
  commandOpts.addEnumOption("qenum3", "Enum Passed as Mutated Index", 0, strings, values, true, false);
  commandOpts.addRadioButtonGroupOption("qradio1", "Radio Passed as Index", 0, strings, 0, true, false);
  commandOpts.addRadioButtonGroupOption("qradio2", "Radio Passed as String", 0, strings, 0, true, true);
  commandOpts.addRadioButtonGroupOption("qradio3", "Radio Passed as Mutated Index", 0, strings, values, true, false);
  commandOpts.addStringOption("qstring", "String", "hello", true, AL::maya::utils::CommandGuiHelper::kStringMustHaveValue);
  commandOpts.addFilePathOption("fp1", "File Path Load", AL::maya::utils::CommandGuiHelper::kLoad, "All Files (*.*) (*.*)", AL::maya::utils::CommandGuiHelper::kStringMustHaveValue);
  commandOpts.addFilePathOption("fp2", "File Path Save", AL::maya::utils::CommandGuiHelper::kSave, "All Files (*.*) (*.*)", AL::maya::utils::CommandGuiHelper::kStringMustHaveValue);
  commandOpts.addFilePathOption("fp3", "File Path Dir", AL::maya::utils::CommandGuiHelper::kDirectory, "All Files (*.*) (*.*)", AL::maya::utils::CommandGuiHelper::kStringMustHaveValue);
  commandOpts.addFilePathOption("fp4", "File Path Dir + File", AL::maya::utils::CommandGuiHelper::kDirectoryWithFiles, "All Files (*.*) (*.*)", AL::maya::utils::CommandGuiHelper::kStringMustHaveValue);
  commandOpts.addFilePathOption("fp5", "File Path Multi File", AL::maya::utils::CommandGuiHelper::kMultiLoad, "All Files (*.*) (*.*)", AL::maya::utils::CommandGuiHelper::kStringMustHaveValue);
}
...
MSyntax CommandGuiHelperTestCMD::createSyntax()
{
  ...
  AL_MAYA_CHECK_ERROR2(syn.addFlag("-wi2", "-qint2", MSyntax::kLong, MSyntax::kLong), "syntaxError");
  AL_MAYA_CHECK_ERROR2(syn.addFlag("-wi3", "-qint3", MSyntax::kLong, MSyntax::kLong, MSyntax::kLong), "syntaxError");
  AL_MAYA_CHECK_ERROR2(syn.addFlag("-wi4", "-qint4", MSyntax::kLong, MSyntax::kLong, MSyntax::kLong, MSyntax::kLong), "syntaxError");
  ...
  AL_MAYA_CHECK_ERROR2(syn.addFlag("-wd2", "-qdouble2", MSyntax::kDouble, MSyntax::kDouble), "syntaxError");
  AL_MAYA_CHECK_ERROR2(syn.addFlag("-wd3", "-qdouble3", MSyntax::kDouble, MSyntax::kDouble, MSyntax::kDouble), "syntaxError");
  AL_MAYA_CHECK_ERROR2(syn.addFlag("-wd4", "-qdouble4", MSyntax::kDouble, MSyntax::kDouble, MSyntax::kDouble, MSyntax::kDouble), "syntaxError");
  AL_MAYA_CHECK_ERROR2(syn.addFlag("-wc", "-qcolour", MSyntax::kDouble, MSyntax::kDouble, MSyntax::kDouble), "syntaxError");
...
//----------------------------------------------------------------------------------------------------------------------
void* CommandGuiHelperTestCMD::creator()
{
  return new CommandGuiHelperTestCMD;
}
...
MStatus CommandGuiHelperTestCMD::doIt(const MArgList& args)
{
  ...
  if(argData.isFlagSet("qdouble4"))
  {
    ...
    std::cout << "qdouble4 " << d4[0] << " " << d4[1] << " " << d4[2] << " " << d4[3] << std::endl;
  }
  else
  {
...
bool test_CommandGuiHelper()
{
  ...
  {
    if(MGlobal::optionVarExists(polyCube_constructionHistory)) MGlobal::removeOptionVar(polyCube_constructionHistory);
    if(MGlobal::optionVarExists(polyCube_width)) MGlobal::removeOptionVar(polyCube_width);
    if(MGlobal::optionVarExists(polyCube_height)) MGlobal::removeOptionVar(polyCube_height);
    if(MGlobal::optionVarExists(polyCube_depth)) MGlobal::removeOptionVar(polyCube_depth);
    if(MGlobal::optionVarExists(polyCube_subdivisionsX)) MGlobal::removeOptionVar(polyCube_subdivisionsX);
    if(MGlobal::optionVarExists(polyCube_subdivisionsY)) MGlobal::removeOptionVar(polyCube_subdivisionsY);
    if(MGlobal::optionVarExists(polyCube_subdivisionsZ)) MGlobal::removeOptionVar(polyCube_subdivisionsZ);
    if(MGlobal::optionVarExists(polyCube_name)) MGlobal::removeOptionVar(polyCube_name);
    if(MGlobal::optionVarExists(polyCube_axis)) MGlobal::removeOptionVar(polyCube_name);
  }

  // generate the GUI.
  {
    // see: http://help.autodesk.com/cloudhelp/2016/ENU/Maya-Tech-Docs/Commands/polyCube.html
    AL::maya::utils::CommandGuiHelper options("polyCube", "Create Polygon Cube", "Create", "USD/polygons/Create Cube");
  ...
  {
    AL::maya::utils::CommandGuiHelper options("pointLight", "Create Point Light", "Create", "USD/lights/Create Point Light");
  ...
  if(0)
  {
    ...
    if(MGlobal::optionVarExists(polyCube_subdivisionsZ)) MGlobal::removeOptionVar(polyCube_subdivisionsZ);
    if(MGlobal::optionVarExists(polyCube_name)) MGlobal::removeOptionVar(polyCube_name);
    if(MGlobal::optionVarExists(polyCube_axis)) MGlobal::removeOptionVar(polyCube_axis);
  }
  return result;
}
