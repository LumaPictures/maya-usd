...
TEST(maya_NodeHelper, addStringAttr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addStringAttr(m_node, longName, shortName, flags));
...
TEST(maya_NodeHelper, addFilePathAttr)
{
  ...
  const uint32_t flags = kCached | kReadable | kWritable | kStorable;
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addFilePathAttr(m_node, longName, shortName, flags, AL::maya::utils::NodeHelper::kSave, "All files (*.*) (*.*)"));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
  tearDown();
}
...
TEST(maya_NodeHelper, addInt8Attr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addInt8Attr(m_node, longName, shortName, defaultValue, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
  EXPECT_EQ(char(defaultValue), findPlug(longName).asChar());

  // This test will always fail, so don't run it. It turns out that if you create a char attribute type in Maya,
  // it will always set a min value of 0, and a max value of 255 (which is pretty dumb given that char is a signed type)
  if(0)
  {
...
TEST(maya_NodeHelper, addInt16Attr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addInt16Attr(m_node, longName, shortName, defaultValue, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
...
TEST(maya_NodeHelper, addInt32Attr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addInt32Attr(m_node, longName, shortName, defaultValue, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
  EXPECT_EQ(defaultValue, findPlug(longName).asInt());
  {
...
TEST(maya_NodeHelper, addInt64Attr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addInt64Attr(m_node, longName, shortName, defaultValue, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
...
TEST(maya_NodeHelper, addFloatAttr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addFloatAttr(m_node, longName, shortName, defaultValue, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
  ...
  {
    AL::maya::utils::NodeHelper::setMinMax(findPlug(longName).attribute(), 4.0f, 40.0f, 4.1f, 39.0f);
    MFnNumericAttribute fn(findPlug(longName).attribute());
    EXPECT_TRUE(fn.hasMin());
    EXPECT_TRUE(fn.hasMax());
    EXPECT_TRUE(fn.hasSoftMax());
...
TEST(maya_NodeHelper, addDoubleAttr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addDoubleAttr(m_node, longName, shortName, defaultValue, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
...
TEST(maya_NodeHelper, addTimeAttr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addTimeAttr(m_node, longName, shortName, defaultValue, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
...
TEST(maya_NodeHelper, addBoolAttr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addBoolAttr(m_node, longName, shortName, defaultValue, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
...
TEST(maya_NodeHelper, addFloat3Attr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addFloat3Attr(m_node, longName, shortName, defaultValueX, defaultValueY, defaultValueZ, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
...
TEST(maya_NodeHelper, addAngle3Attr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addAngle3Attr(m_node, longName, shortName, defaultValueX, defaultValueY, defaultValueZ, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
  EXPECT_NEAR(defaultValueX, findPlug(longName).child(0).asFloat(), 1e-5f);
  EXPECT_NEAR(defaultValueY, findPlug(longName).child(1).asFloat(), 1e-5f);
...
TEST(maya_NodeHelper, addPointAttr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addPointAttr(m_node, longName, shortName, MPoint(defaultValueX, defaultValueY, defaultValueZ), flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
...
TEST(maya_NodeHelper, addVectorAttr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addVectorAttr(m_node, longName, shortName, MVector(defaultValueX, defaultValueY, defaultValueZ), flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
  EXPECT_NEAR(defaultValueX, findPlug(longName).child(0).asDouble(), 1e-5f);
  EXPECT_NEAR(defaultValueY, findPlug(longName).child(1).asDouble(), 1e-5f);
...
TEST(maya_NodeHelper, addFloatPointAttr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addFloatPointAttr(m_node, longName, shortName, MFloatPoint(defaultValueX, defaultValueY, defaultValueZ), flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
  EXPECT_NEAR(defaultValueX, findPlug(longName).child(0).asFloat(), 1e-5f);
  EXPECT_NEAR(defaultValueY, findPlug(longName).child(1).asFloat(), 1e-5f);
...
TEST(maya_NodeHelper, addFloatVectorAttr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addFloatVectorAttr(m_node, longName, shortName, MFloatVector(defaultValueX, defaultValueY, defaultValueZ), flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
  EXPECT_NEAR(defaultValueX, findPlug(longName).child(0).asFloat(), 1e-5f);
  EXPECT_NEAR(defaultValueY, findPlug(longName).child(1).asFloat(), 1e-5f);
...
TEST(maya_NodeHelper, addColourAttr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addColourAttr(m_node, longName, shortName, MColor(defaultValueX, defaultValueY, defaultValueZ, defaultValueW), flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
  EXPECT_NEAR(defaultValueX, findPlug(longName).child(0).asFloat(), 1e-5f);
  EXPECT_NEAR(defaultValueY, findPlug(longName).child(1).asFloat(), 1e-5f);
...
TEST(maya_NodeHelper, addMatrixAttr)
{
  ...
  const uint32_t flags = kCached | kReadable | kWritable | kStorable;
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addMatrixAttr(m_node, longName, shortName, MM, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
  MObject attrData;
...
TEST(maya_NodeHelper, addDataAttr)
{
  ...
  const uint32_t flags = kCached | kReadable | kWritable | kHidden | kStorable;
  {
    ...
    EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addDataAttr(m_node, longName, shortName, MFnData::kVectorArray, flags, behaviour));
    EXPECT_EQ(findPlug(longName), findPlug(shortName));
    checkAttributeFlags(findPlug(longName), flags);
  ...
  }
  {
    ...
    EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addDataAttr(m_node, longName, shortName, MFnData::kMesh, flags, behaviour));
    EXPECT_EQ(findPlug(longName), findPlug(shortName));
    checkAttributeFlags(findPlug(longName), flags);
    MFnAttribute fn(findPlug(longName).attribute());
    EXPECT_EQ(behaviour, fn.disconnectBehavior());
  ...
  }
  {
    ...
    EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addDataAttr(m_node, longName, shortName, MFnData::kLattice, flags, behaviour));
    EXPECT_EQ(findPlug(longName), findPlug(shortName));
    checkAttributeFlags(findPlug(longName), flags);
    MFnAttribute fn(findPlug(longName).attribute());
    EXPECT_EQ(behaviour, fn.disconnectBehavior());
...
TEST(maya_NodeHelper, addMessageAttr)
{
  ...
  EXPECT_EQ(MStatus(MS::kSuccess), AL::maya::utils::NodeHelper::addMessageAttr(m_node, longName, shortName, flags));
  EXPECT_EQ(findPlug(longName), findPlug(shortName));
  checkAttributeFlags(findPlug(longName), flags);
  tearDown();
}
...
TEST(maya_NodeHelper, testAttributeFlags)
{
  ...
  /// \todo Investigate.
  ///       So when I'm *not* setting kUsesArrayDataBuilder, kUsesArrayDataBuilder gets set anyway whenever you use kArray? WAT?
  #if 0
  {
