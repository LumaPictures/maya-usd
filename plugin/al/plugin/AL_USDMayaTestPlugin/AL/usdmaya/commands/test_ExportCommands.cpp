...
TEST(ExportCommands, exportUVOnly)
{
  ...
  MString exportCmd;
  exportCmd.format(MString("AL_usdmaya_ExportCommand -f \"^1s\" -sl 1 -uvo 1 -luv 1"), AL::maya::utils::convert(temp_path));
...
TEST(ExportCommands, exportUVS)
{
  ...
  MGlobal::executeCommand(MString("createNode transform -n geo;polyCube -n cube -cuv 2;parent cube geo;select geo"), false, true);
  MString exportCmd;
  exportCmd.format(MString("AL_usdmaya_ExportCommand -f \"^1s\" -sl 1 -uvs 1 -luv 1"), AL::maya::utils::convert(temp_path_withUV));
  MGlobal::executeCommand(exportCmd, true);
  ...
  exportCmd.format(MString("AL_usdmaya_ExportCommand -f \"^1s\" -sl 1 -uvs 0 -luv 1"), AL::maya::utils::convert(temp_path_withoutUV));
  MGlobal::executeCommand(exportCmd, true);
