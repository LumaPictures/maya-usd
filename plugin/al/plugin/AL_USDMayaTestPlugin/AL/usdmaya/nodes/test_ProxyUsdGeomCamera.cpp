...
TEST(ProxyUsdGeomCamera, cameraProxyReadWriteAttributes)
{
  ...
  const std::string temp_path = buildTempPath("AL_USDMayaTests_cameraProxyReadWriteAttributes.usda");
  ...
  MGlobal::executeCommand("connectAttr \"" + proxyShapeName + ".outStageData\" \"" + proxyCameraName + ".stage\";");
  ...
  // Maya -> USD
  proxyCamera->projectionPlug().setInt(static_cast<uint16_t>(ProxyUsdGeomCamera::Projection::Perspective));
  TfToken projection;
  ...
  proxyCamera->projectionPlug().setInt(static_cast<uint16_t>(ProxyUsdGeomCamera::Projection::Orthographic));
  camera.GetProjectionAttr().Get(&projection, usdTime);
  ...
  EXPECT_EQ(static_cast<uint16_t>(ProxyUsdGeomCamera::Projection::Perspective), proxyCamera->projectionPlug().asShort());
  EXPECT_EQ(false, proxyCamera->orthographicPlug().asBool());

  camera.GetProjectionAttr().Set(UsdGeomTokens->orthographic, usdTime);
  EXPECT_EQ(static_cast<uint16_t>(ProxyUsdGeomCamera::Projection::Orthographic), proxyCamera->projectionPlug().asShort());
  EXPECT_EQ(true, proxyCamera->orthographicPlug().asBool());


  // Maya -> USD
  proxyCamera->stereoRolePlug().setInt(static_cast<uint16_t>(ProxyUsdGeomCamera::StereoRole::Left));
  TfToken stereoRole;
  camera.GetStereoRoleAttr().Get(&stereoRole, usdTime);
  EXPECT_EQ(UsdGeomTokens->left, stereoRole);

  proxyCamera->stereoRolePlug().setInt(static_cast<uint16_t>(ProxyUsdGeomCamera::StereoRole::Mono));
  camera.GetStereoRoleAttr().Get(&stereoRole, usdTime);
  EXPECT_EQ(UsdGeomTokens->mono, stereoRole);

  // USD -> Maya
  camera.GetStereoRoleAttr().Set(UsdGeomTokens->left, UsdTimeCode::Default());
  EXPECT_EQ(static_cast<uint16_t>(ProxyUsdGeomCamera::StereoRole::Left), proxyCamera->stereoRolePlug().asShort());

  camera.GetStereoRoleAttr().Set(UsdGeomTokens->mono, UsdTimeCode::Default());
  EXPECT_EQ(static_cast<uint16_t>(ProxyUsdGeomCamera::StereoRole::Mono), proxyCamera->stereoRolePlug().asShort());
