...
TEST(LockPrims, lockMetaDataTranslatePrim)
{
  ...
  ASSERT_TRUE(MGlobal::executeCommand("AL_usdmaya_TranslatePrim -fi -ip \"/hello/world/cam\" -p \"proxy\""));

  EXPECT_TRUE(checkUnlocked("hello"));
  EXPECT_TRUE(checkLocked("world"));
  EXPECT_TRUE(checkLocked("cam"));

  ASSERT_TRUE(MGlobal::executeCommand("AL_usdmaya_TranslatePrim -tp \"/hello/world/cam\" -p \"proxy\""));
...
// This test loads a usd file that contains variants for all permutations of the selectable meta data. 
// Each variant is selected in turn, and hopefully the selectability DB within the proxy shape has 
