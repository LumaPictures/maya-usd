...
TEST(Transform, splitPivot)
{
  ...
  const std::string temp_path = buildTempPath("AL_USDMayaTests_transform_splitPivot.usda");
  std::string sessionLayerContents;
...

  {
    ...
    AL::usdmaya::nodes::Transform* transformNode = (AL::usdmaya::nodes::Transform*)xformMfn.userNode();
    transformNode->pushToPrimPlug().setValue(true);
