...
MStatus FrameRange::setFrameRange(const UsdPrim& prim, bool setCurrentFrame)
{
  ...
  MStatus animStartStat = getFrame(stage, prim, animStartAttr, startTimeCodeKey, &animationStartFrame);
  MStatus animEndStat = getFrame(stage, prim, animEndAttr, endTimeCodeKey, &animationEndFrame);

  MStatus visibleStartStat = getFrame(stage, prim, visibleStartAttr, startTimeCodeKey, &visibleStartFrame);
  MStatus visibleEndStat = getFrame(stage, prim, visibleEndAttr, endTimeCodeKey, &visibleEndFrame);

  MStatus currentFrameStat = getFrame(stage, prim, currentFrameAttr, startTimeCodeKey, &currentFrame);
  ...
  if(hasFrameRange)
  {
    ...
    TF_DEBUG(ALUSDMAYA_TRANSLATORS).Msg("FrameRange::setFrameRange(%f, %f, %f, %f) on prim %s\n",
                                        animationStartFrame, visibleStartFrame, visibleEndFrame, animationEndFrame,
  ...
  if(setCurrentFrame)
  {
    ...
    if(hasCurrentFrame)
    {
      ...
      TF_DEBUG(ALUSDMAYA_TRANSLATORS).Msg("FrameRange::setCurrentFrame(%f) on prim %s\n",
                                          currentFrame, prim.GetPath().GetText());
