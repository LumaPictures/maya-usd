...
{
  if(obj == MObject::kNullObj) os << "kNullObj";
  else
    os << obj.apiTypeStr();
  return os;
}
...
///         values to it, export, re-import, and then see if the imported matches the exported.
...
inline void randomInt16(MPlug plug)
{
  ...
  TRACE(std::cout << "randomInt16 " << plug.name().asChar() << " " << plug.asShort() << std::endl);
}
...
inline void randomInt64(MPlug plug)
{
  ...
  TRACE(std::cout << "randomInt64 " << plug.name().asChar() << " " << plug.asInt64() << std::endl);
}
inline void randomFloat(MPlug plug)
{
  EXPECT_EQ(MStatus(MS::kSuccess), plug.setFloat(randFloat()));
  TRACE(std::cout << "randomFloat " << plug.name().asChar() << " " << plug.asFloat() << std::endl);
}
inline void randomDouble(MPlug plug)
{
  EXPECT_EQ(MStatus(MS::kSuccess), plug.setDouble(randDouble()));
  TRACE(std::cout << "randomDouble " << plug.name().asChar() << " " << plug.asDouble() << std::endl);
}
inline void randomAngle(MPlug plug)
{
  ...
  TRACE(std::cout << "randomAngle " << plug.name().asChar() << " " << plug.asDouble() << std::endl);
}
inline void randomTime(MPlug plug)
{
  MTime mtime(randDouble(), MTime::kSeconds);
  EXPECT_EQ(MStatus(MS::kSuccess), plug.setMTime(mtime));
  TRACE(std::cout << "randomTime " << plug.name().asChar() << " " << plug.asDouble() << std::endl);
}
inline void randomDistance(MPlug plug)
{
  ...
  TRACE(std::cout << "randomDistance " << plug.name().asChar() << " " << plug.asDouble() << std::endl);
}
inline void randomString(MPlug plug)
{
  ...
  TRACE(std::cout << "randomString " << plug.name().asChar() << " " << plug.asString() << std::endl);
}
