...
TF_DEFINE_PUBLIC_TOKENS(UsdMayaReferenceAssemblyTokens, PXRUSDMAYA_REFERENCE_ASSEMBLY_TOKENS);

TF_DEFINE_PUBLIC_TOKENS(UsdMayaVariantSetTokens, PXRUSDMAYA_VARIANT_SET_TOKENS);

TF_DEFINE_ENV_SETTING(
    PIXMAYA_USE_USD_ASSEM_NAMESPACE,
    true,
    "Prefixes unrolled USD assemblies with namespaces");

bool UsdMayaUseUsdAssemblyNamespace() { return TfGetEnvSetting(PIXMAYA_USE_USD_ASSEM_NAMESPACE); }

// == Statics ==
const MTypeId UsdMayaReferenceAssembly::typeId(0x0010A251);
const MString
              UsdMayaReferenceAssembly::typeName(UsdMayaReferenceAssemblyTokens->MayaTypeName.GetText());
...
/* static */
void* UsdMayaReferenceAssembly::creator() { return new UsdMayaReferenceAssembly(); }
...
MStatus UsdMayaReferenceAssembly::initialize()
{
    ...
    filePathAttr
        = typedAttrFn.create("filePath", "fp", MFnData::kString, MObject::kNullObj, &status);
    ...
    primPathAttr
        = typedAttrFn.create("primPath", "pp", MFnData::kString, MObject::kNullObj, &status);
    CHECK_MSTATUS_AND_RETURN_IT(status);
    typedAttrFn.setInternal(true); // trigger getInternalValueInContext() on change
    ...
    excludePrimPathsAttr = typedAttrFn.create(
        "excludePrimPaths", "epp", MFnData::kString, MObject::kNullObj, &status);
    CHECK_MSTATUS_AND_RETURN_IT(status);
    ...
    kindAttr = typedAttrFn.create("kind", "knd", MFnData::kString, MObject::kNullObj, &status);
    CHECK_MSTATUS_AND_RETURN_IT(status);
    ...
    initialRepAttr
        = typedAttrFn.create("initialRep", "irp", MFnData::kString, MObject::kNullObj, &status);
    addAttribute(initialRepAttr);

    inStageDataAttr = typedAttrFn.create(
        "inStageData", "id", MayaUsdStageData::mayaTypeId, MObject::kNullObj, &status);
    CHECK_MSTATUS_AND_RETURN_IT(status);
    ...
    repNamespaceAttr
        = typedAttrFn.create("repNamespace", "rns", MFnData::kString, MObject::kNullObj, &status);
    CHECK_MSTATUS_AND_RETURN_IT(status);
    ...
    drawModeAttr
        = typedAttrFn.create("drawMode", "dm", MFnData::kString, MObject::kNullObj, &status);
    CHECK_MSTATUS_AND_RETURN_IT(status);
    typedAttrFn.setReadable(false);
    typedAttrFn.setStorable(false);
    ...
    inStageDataCachedAttr = typedAttrFn.create(
        "inStageDataCached", "idc", MayaUsdStageData::mayaTypeId, MObject::kNullObj, &status);
    CHECK_MSTATUS_AND_RETURN_IT(status);
    typedAttrFn.setStorable(false);
    typedAttrFn.setWritable(false);
    ...
    outStageDataAttr = typedAttrFn.create(
        "outStageData", "od", MayaUsdStageData::mayaTypeId, MObject::kNullObj, &status);
    CHECK_MSTATUS_AND_RETURN_IT(status);
    typedAttrFn.setStorable(false);
    typedAttrFn.setWritable(false);
    ...
    , _hasEdits(false)
{
    ...
    _representations[std::string(UsdMayaRepresentationCollapsed::_assemblyType.asChar())]
        = std::make_shared<UsdMayaRepresentationCollapsed>(
            this, UsdMayaRepresentationCollapsed::_assemblyType);

    _representations[std::string(UsdMayaRepresentationCards::_assemblyType.asChar())]
        = std::make_shared<UsdMayaRepresentationCards>(
            this, UsdMayaRepresentationCards::_assemblyType);

    _representations[std::string(UsdMayaRepresentationPlayback::_assemblyType.asChar())]
        = std::make_shared<UsdMayaRepresentationPlayback>(
            this, UsdMayaRepresentationPlayback::_assemblyType);

    _representations[std::string(UsdMayaRepresentationExpanded::_assemblyType.asChar())]
        = std::make_shared<UsdMayaRepresentationExpanded>(
            this, UsdMayaRepresentationExpanded::_assemblyType);

    _representations[std::string(UsdMayaRepresentationFull::_assemblyType.asChar())]
        = std::make_shared<UsdMayaRepresentationFull>(
            this, UsdMayaRepresentationFull::_assemblyType);
}
...
MString UsdMayaReferenceAssembly::getRepLabel(const MString& rep) const { return getRepType(rep); }
...
MString UsdMayaReferenceAssembly::setRepName(
    const MString& /*representation*/,
...
MStatus
UsdMayaReferenceAssembly::setRepLabel(const MString& /*representation*/, const MString& /*label*/)
{
    return MS::kFailure;
}
...
void UsdMayaReferenceAssembly::postLoad()
{
    ...
    if (_activateRepOnFileLoad) {
        // logging.debug("In postLoad activate: isTopLevel=%r
        // canActivate=%r"%(assemblyFn.isTopLevel(), assemblyFn.canActivate()))
        if (assemblyFn
                .canActivate()) { // Consider adding assemblyFn.isTopLevel() to the conditional
            MPlug   initialRepPlg(thisMObject(), initialRepAttr);
...
MString UsdMayaReferenceAssembly::getDefaultRepNamespace() const
{
...
/* virtual */
MString UsdMayaReferenceAssembly::getRepNamespace() const
{
...
/* virtual */
void UsdMayaReferenceAssembly::updateRepNamespace(const MString& repNamespace)
{
    // This was adapted from Autodesk's sample assembly reference node:
    //
    // http://help.autodesk.com/view/MAYAUL/2017/ENU/?guid=__cpp_ref_scene_assembly_2assembly_reference_8cpp_example_html
...
// virtual
MStatus
UsdMayaReferenceAssembly::setDependentsDirty(const MPlug& dirtiedPlug, MPlugArray& affectedPlugs)
{
    ...
    if ((dirtiedPlugName.length() > variantSetPrefix.length())
        ...
        TF_STATUS("Dirtying stage due to variant selection change: %s", dirtiedPlugName.asChar());
...
MStatus
UsdMayaReferenceAssembly::connectionMade(const MPlug& plug, const MPlug& otherPlug, bool asSrc)
{
...
MStatus
UsdMayaReferenceAssembly::connectionBroken(const MPlug& plug, const MPlug& otherPlug, bool asSrc)
{
    if (asSrc && otherPlug.node().hasFn(MFn::kInstancer)) {
        const MObject assembly = plug.node();
        const MObject instancer = otherPlug.node();
        UsdMayaAssemblyDisconnectedFromInstancerNotice(assembly, instancer).Send();
    }
    return MPxAssembly::connectionBroken(plug, otherPlug, asSrc);
}

MStatus UsdMayaReferenceAssembly::compute(const MPlug& aPlug, MDataBlock& dataBlock)
{
...
static MItEdits _GetEdits(const MObject& assemObj)
{
...
static std::set<std::string> _GetVariantSetNamesForStageCache(const MFnDependencyNode& depNodeFn)
{
    ...
    std::transform(
        ...
        [](const UsdUtilsRegisteredVariantSet& varSet) { return varSet.name; });
...
MStatus UsdMayaReferenceAssembly::computeInStageDataCached(MDataBlock& dataBlock)
{
    ...
    if (!inDataHandle.data().isNull()) {
        ...
        if (SdfLayerRefPtr rootLayer = SdfLayer::FindOrOpen(fileString)) {
            ...
            const std::set<std::string> varSetNamesForCache
                = _GetVariantSetNamesForStageCache(dagNodeFn);
            ...
            SdfLayerRefPtr sessionLayer = UsdMayaStageCache::GetSharedSessionLayer(
                SdfPath::AbsoluteRootPath().AppendChild(modelName), varSels, drawMode);
            ...
            primPath = usdStage->GetDefaultPrim() ? usdStage->GetDefaultPrim().GetPath() :

                                                  // XXX:
                                                  // Preserving prior behavior for now-- eventually
                                                  // might make more sense to bail in this case.
...
MStatus UsdMayaReferenceAssembly::computeOutStageData(MDataBlock& dataBlock)
{
    ...
    if (!usdPrim && !primPathStr.empty()) {
        ...
        if (primPath.HasPrefix(inData->primPath)) {
            ...
            TF_WARN(
                "%s: Assembly primPath <%s> is not a descendant of input "
    ...
    if (usdPrim && !assemblyFn.isTopLevel()) {
        ...
        TF_FOR_ALL(variantSet, variantSetNames)
        {
            ...
            if (!varSetPlg.isNull()) {
                ...
                std::string existingVarSelect
                    = usdPrim.GetVariantSet(*variantSet).GetVariantSelection();
        ...
        if (!drawModePlug.isNull()) {
            ...
            TfToken existingDrawMode = UsdGeomModelAPI(usdPrim).ComputeModelDrawMode();
        ...
        if (!varSets.empty() || !drawMode.IsEmpty()) {
            SdfLayerRefPtr newLayer
                = UsdMayaStageCache::GetSharedSessionLayer(usdPrim.GetPath(), varSets, drawMode);
    ...
    stageData->primPath = usdPrim ? usdPrim.GetPath() : SdfPath::AbsoluteRootPath();
...
bool UsdMayaReferenceAssembly::setInternalValueInContext(
    const MPlug&       plug,
    ...
    MDGContext&        ctx)
{
    ...
    bool varSelChanged
        = TfStringStartsWith(plug.partialName().asChar(), UsdMayaVariantSetTokens->PlugNamePrefix);

    if (varSelChanged
        || (std::find(
                attrsAffectingRepresentation.begin(),
...
UsdPrim UsdMayaReferenceAssembly::usdPrim() const
{
    ...
    usdPrim = outData->primPath.IsEmpty()
        ? (outData->stage->GetDefaultPrim() ? outData->stage->GetDefaultPrim()
...
void UsdMayaReferenceAssembly::ConnectMayaTimeToAssemblyTime()
{
    MFnAssembly assemblyFn(thisMObject());
...
void UsdMayaReferenceAssembly::DisconnectAssemblyTimeFromMayaTime()
{
    MFnAssembly assemblyFn(thisMObject());
    MPlug       assemblyTimePlug = assemblyFn.findPlug(timeAttr, true);
    if (!assemblyTimePlug || !assemblyTimePlug.isConnected()) {
...
UsdMayaRepresentationBase::UsdMayaRepresentationBase(MPxAssembly* assembly, const MString& name)
...
bool UsdMayaRepresentationProxyBase::activate()
{
    ...
    dgMod.newPlugValueBool(
        shapeFn.findPlug(UsdMayaProxyShape::softSelectableAttr, true), _proxyIsSoftSelectable);
...
void UsdMayaRepresentationProxyBase::_PushEditsToProxy()
{
    ...
    if (!assemEdits.empty()) {
        ...
        stage->GetSessionLayer()->GetSubLayerPaths().push_back(_sessionSublayer->GetIdentifier());
    ...
    MDGModifier&       dgMod)
{
    ...
    dgMod.connect(
        assemblyFn.findPlug(UsdMayaReferenceAssembly::timeAttr, true),
        shapeFn.findPlug(UsdMayaProxyShape::timeAttr, true));

    dgMod.connect(
        assemblyFn.findPlug(UsdMayaReferenceAssembly::excludePrimPathsAttr, true),
        shapeFn.findPlug(UsdMayaProxyShape::excludePrimPathsAttr, true));
}

void UsdMayaRepresentationCollapsed::_OverrideProxyPlugs(
    MFnDependencyNode& shapeFn,
    MDGModifier&       dgMod)
{
    dgMod.newPlugValueBool(shapeFn.findPlug(UsdMayaProxyShape::fastPlaybackAttr, true), false);

    // Call parent for common proxy overrides
    UsdMayaRepresentationProxyBase::_OverrideProxyPlugs(shapeFn, dgMod);
}

bool UsdMayaRepresentationCards::activate()
{
    MFnDagNode dagFn(getAssembly()->thisMObject());
...
bool UsdMayaRepresentationCards::inactivate()
{
    MFnDagNode dagFn(getAssembly()->thisMObject());
    MPlug      drawMode = dagFn.findPlug(UsdMayaReferenceAssembly::drawModeAttr, true);
    drawMode.setString("");
...
void UsdMayaRepresentationCards::_OverrideProxyPlugs(MFnDependencyNode& shapeFn, MDGModifier& dgMod)
{
    dgMod.newPlugValueBool(shapeFn.findPlug(UsdMayaProxyShape::fastPlaybackAttr, true), false);
...
/* virtual */
bool UsdMayaRepresentationPlayback::activate()
{
    UsdMayaReferenceAssembly* usdAssembly = dynamic_cast<UsdMayaReferenceAssembly*>(getAssembly());
...
/* virtual */
bool UsdMayaRepresentationPlayback::inactivate()
{
    UsdMayaReferenceAssembly* usdAssembly = dynamic_cast<UsdMayaReferenceAssembly*>(getAssembly());
...
void UsdMayaRepresentationPlayback::_OverrideProxyPlugs(
    MFnDependencyNode& shapeFn,
    MDGModifier&       dgMod)
{
    dgMod.newPlugValueBool(shapeFn.findPlug(UsdMayaProxyShape::fastPlaybackAttr, true), true);
...
void UsdMayaRepresentationHierBase::_ConnectSubAssemblyPlugs()
{
    MStatus status;
...
void UsdMayaRepresentationHierBase::_ConnectProxyPlugs()
{
    MStatus status;
...
bool UsdMayaRepresentationHierBase::activate()
{
    ...
    MFnAssembly assemblyFn(getAssembly()->thisMObject());
    MString     usdFilePath(
        assemblyFn.findPlug(UsdMayaReferenceAssembly::filePathAttr, true).asString());
    MString usdPrimPath(
        assemblyFn.findPlug(UsdMayaReferenceAssembly::primPathAttr, true).asString());
    ...
    UsdMayaJobImportArgs importArgs = UsdMayaJobImportArgs::CreateFromDictionary(
    ...
    UsdMaya_ReadJobWithSceneAssembly readJob(importData, importArgs);
...
bool UsdMayaGL_InstancerImager_ContinueTrackingOnDisconnect(const MFnDependencyNode& fn)
{
